...
function generate_job_log_json() {
  ...
  pipeline_duration=$(( (pipeline_end_timestamp - d2) / 86400 ))
...
function write_report() {
  ...
  echo "$job_log_json" > tests/legacy_observability/history/"$CONNECTOR"/"$GIT_REVISION".json

  aws s3 sync "$BUCKET_WRITE_ROOT"/tests/legacy_observability/history/"$CONNECTOR"/ s3://"$BUCKET"/legacy_observability/tests/history/"$CONNECTOR"/
}
